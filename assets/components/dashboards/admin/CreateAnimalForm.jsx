...
const CreateAnimalForm = (props) => {
  ...
  useEffect(() => {
    const postImagesData = async () => {
      if (imagesLoaded && newAnimalId) {
        try {
          await animalsApi.createAnimalImages(
            imagesData,
            parseInt(newAnimalId, 10)
          );
        } catch (error) {
          await animalsApi.deleteAnimal(newAnimalId);
          setErrors({
            images: "Une erreur est survenue lors de la création des images.",
          });
        }
      }
    };
    postImagesData();
  }, [imagesLoaded, newAnimalId]);
